...
ANTSSCRIPTNAME=${ANTSPATH}/antsIntroductionRegularizedJohn.sh
...
Optional arguments:
     ...
     -e: Type of regularization used for registration.
...
while getopts "c:q:d:g:i:j:h:m:n:o:p:s:r:t:x:z:e:" OPT
  do
  case $OPT in
      h) #help
	  echo "$USAGE"
...
if [ "$RIGID" -eq 1 ] ;
    ...
    for IMG in $IMAGESETVARIABLE
      ...
      if [ $DOQSUB -eq 1 ] ; then
		id=`qsub -pe batch 4 -cwd -S /bin/bash -N antsBuildTemplate_rigid -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=4,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH $QSUBOPTS $qscript | awk '{print $3}'`
		jobIDs="$jobIDs $id"
        sleep 0.5
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE
    ...
    if [ $DOQSUB -eq 1 ]; then
      # Use 4 threads!
      id=`qsub -pe batch 4 -cwd -N antsBuildTemplate_deformable_${i} -S /bin/bash -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=4,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH $QSUBOPTS $exe | awk '{print $3}'`
      jobIDs="$jobIDs $id"
      sleep 0.5
    elif [ $DOQSUB -eq 4 ]; then
...
#		echo " submitting job number $count " # for debugging only
  ...
  echo
  echo "--------------------------------------------------------------------------------------"
  echo " Shape update by partition"
  echo "--------------------------------------------------------------------------------------"
  /groups/saalfeld/home/bogovicj/dev/template/template-building/antsScripts/shapeUpdateByPartition.sh ${DIM} ${TEMPLATE} ${TEMPLATENAME} ${OUTPUTNAME} ${GRADIENTSTEP} "-F-" "-M-"
