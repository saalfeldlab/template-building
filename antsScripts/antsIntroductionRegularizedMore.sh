...
while getopts "d:f:r:i:h:o:m:n:l:q:s:t:e:" OPT
do
    case $OPT in
    h) #help
        echo "$Help"
...
elif [ "${TRANSFORMATIONTYPE}" == "ELrigid" ]
then
# Mapping Parameters
TRANSFORMATION=Elast[1]
#REGULARIZATION=Gauss[3,0.5]
...
echo "Invalid transformation metric. Use RI, RA, EL, ELrigid, SY, S2, GR , DD or EX or type bash `basename $0` -h."
exit 1
fi
...
AFFINEITERATIONS=1000x1000x1000

if  [ ${N4CORRECT} -eq 0 ] && [ ${RIGID} -eq 0 ]
then
# Apply ANTS mapping command without N4BiasFieldCorrection
exe="${ANTSPATH}/ANTS $DIM -m ${METRIC}${FIXED},${MOVING},${METRICPARAMS} -t $TRANSFORMATION -r $REGULARIZATION -o ${OUTPUTNAME} -i $MAXITERATIONS --affine-metric-type $METRICTYPE --number-of-affine-iterations $AFFINEITERATIONS $ISRIGID "
echo
echo "--------------------------------------------------------------------------------------"
echo "ITK THREADS: $ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS"
echo "Janelia mods"
echo "ANTS command:"
echo "$exe "
echo "--------------------------------------------------------------------------------------"
...
exe="${ANTSPATH}/ANTS $DIM -m ${METRIC}${FIXED},${OUTPUTNAME}.nii.gz,${METRICPARAMS} -t $TRANSFORMATION -r $REGULARIZATION -o ${OUTPUTNAME} -i $MAXITERATIONS --affine-metric-type $METRICTYPE --number-of-affine-iterations $AFFINEITERATIONS $ISRIGID "
echo
echo "--------------------------------------------------------------------------------------"
echo "ANTS command:"
echo "Janelia mods"
