...
public class ConfocalStitch
{
	...
	public static boolean defaultSubpixelAccuracy = true; // ONE OF TWO CHANGES TO DEFAULTS THAT I MADE -JB
	...
	public static int defaultResult = 1; // ONE OF TWO CHANGES TO DEFAULTS THAT I MADE -JB
	...
	public static void main( String[] args ) 
	{
		...
		for( String s : args )
		{
			...
			ConfocalStitch cs = new ConfocalStitch();
			cs.run( dir, tc, dir, true );
	...
	public File run( String directory, String tileConfig, String outputDirectory, boolean alsoSaveAvgProj ) 
	{
		Log.info( "Stitching internal version: " + Stitching_Grid.version );

		String outputFile = tileConfig; // have this because the name outputfile is confusing to me
		

		
		final int gridType  = 6;
		final int gridOrder = 0;
		...
		if ( params.fusionMethod != CommonFunctions.fusionMethodListGrid.length - 1 )
		{
			...
			final String outputImageFile = outputFile.replaceAll( ".txt", ".tif" );
			outputFullPath = outputDirectory + File.separator + outputImageFile;

			System.out.println( "outputFullPath: " + outputFullPath );
			
			IJ.save( imp, outputFullPath );
			...
			if( alsoSaveAvgProj )
			{
				Log.info("Computing average projection" );
				...
				ImagePlus avgProjIp = projector.getProjection();
				
				Log.info( " " + avgProjIp );
				String avgProjPath =  outputFullPath.replaceAll( ".tif", "_avgProj.tif" );
				Log.info("Saving to: " + avgProjPath );
				IJ.saveAsTiff( avgProjIp, avgProjPath );
			}
    	...
    	return new File( outputFullPath );
	}
