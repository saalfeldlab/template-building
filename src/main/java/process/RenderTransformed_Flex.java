 ...
 *
 */
public class RenderTransformed_Flex
{
	...
	public static RealTransform loadTransform( String filePath, boolean invert ) throws Exception
	{
		if( filePath.endsWith( "h5" ) || filePath.contains( "h5:" ))
		{
		...
		else if( filePath.endsWith( "csv" ))
		{
			...
			return new ThinplateSplineTransform( ltm.getTransform() );
		}
...
//				return new InverseRealTransform( invxfm );
	...
	public static void testTransform( RealTransform xfm )
	{
		//double[] p = new double[]{ 14.8, 8.4, 4.8 };
		double[] p = new double[]{ 74, 42, 12 };
			...
			int nThreads )
	{
		...
		if( resOutXfm != null )
			totalXfm.add( resOutXfm );
	
		xfm.forEach( x -> totalXfm.add( x ));

		if( resInXfm != null )
			totalXfm.add( resInXfm.inverse() );
		
		testTransform( totalXfm );
...
//		}
			...
			FinalInterval renderInterval ) throws Exception
	{
		...
		ArrayList<RealTransform> totalXfm = new ArrayList<>();
		...
		while( i < args.length )
		{
			...
			RealTransform xfm = loadTransform( args[ i ], invert );
