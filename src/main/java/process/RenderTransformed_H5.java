 ...
 *
 */
public class RenderTransformed_H5
{
...
//				return new InverseRealTransform( invxfm );
			...
			FinalInterval renderInterval ) throws Exception
	{
		...
		while( i < args.length )
		{
			...
			{
				i++;
				N5HDF5Reader n5 = new N5HDF5Reader( args[ i ], 3, 32, 32, 32 );
				xfm = N5DisplacementField.open( n5, "dfield", false );
			...
			else if( args[ i ].equals( "-i" ))
			{
				i++;
				N5HDF5Reader n5 = new N5HDF5Reader( args[ i ], 3, 32, 32, 32 );
				xfm = N5DisplacementField.open( n5, "invdfield", true );
		...
		IntervalView< T > imgHiXfm = Views.interval( 
				Views.raster( 
					new RealTransformRealRandomAccessible<T,RealTransform>(
